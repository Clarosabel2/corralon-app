...
namespace BLL
{
    ...
    {
        public bool CreateLanguage(string userInput)
        {
            return LanguageRepository.CreateLanguage(userInput);
        }
        ...
        {
            DataTable dt = LanguageRepository.GetTranslationsDataTable();
            ...
            SessionManager.translations = new Dictionary<Language, Dictionary<string, Dictionary<string, string>>>();
        ...
        public void SetDefaultLanguage(string selectedItem)
        {
            LanguageRepository.SetDefaultLanguage(selectedItem);
        }

        public bool UpdateTranslations(Tuple<string, string, DataTable> translations)
        {
            var entry = SessionManager.translations.FirstOrDefault(i => i.Key.Name == translations.Item1);
            Language len = entry.Key;
            ...
            foreach (DataRow row in translations.Item3.Rows)
            {
                if (row[1].ToString() != "")
                {
                        ...
                        SessionManager.translations[len][translations.Item2][controlName] != translationValue)
                    {
                        ...
                        LanguageRepository.UpdateTranslation(languageName, formName, controlName, translationValue);
                        anyChanges = true;
                    }   
                    else
                    {
                        // IDIOMA SELECCIONADO, FORMULARIO ACTUAL, DATATABLE CON TRADUCCIONES
                        LanguageRepository.InsertTranslation(languageName, formName, controlName, translationValue);
                        anyChanges = true;
                    }
